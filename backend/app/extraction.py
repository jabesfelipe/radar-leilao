...
def _prompt(document_type: str, context: str) -> list[dict[str, str]]:
    rules = "Extraia somente fatos literalmente presentes. Use null para ausência. Nunca conclua validade, nulidade, risco, consolidação, evicção, regularidade, dívida ou ocupação. Cada referência deve apontar chunk_id/página/seção quando disponíveis."
...
def extract_document(db, property_id: int, document_version_id: int, document_type: str, gateway_override: LLMGateway | None = None) -> ExtractionResult:
    ...
    if not version or version.document.property_id != property_id:
    ...
    try: output = call.content if isinstance(call.content, schema) else schema.model_validate(call.content)
...
def persist_extraction(db, prop: models.Property, result: ExtractionResult):
    version = db.get(models.DocumentVersion, result.document_version_id)
    ...
    for reference in output.references:
        ...
        db.add(models.EvidenceLink(evidence_id=evidence.id, target_type=target_type, target_id=item.id, relation="SUSTENTA"))
    ...
    event = record_event(db, prop, event_type, target_type, item.id, payload, domains)
    record_history(db, prop, target_type, item.id, "CREATE", None, payload, event.id, item.evidence_id)
